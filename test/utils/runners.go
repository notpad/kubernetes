...
func NewWithControllerCreatePodStrategy(controllerName string, podTemplate *v1.Pod) TestPodCreateStrategy {
	return func(client clientset.Interface, namespace string, podCount int) error {
		podTemplate.ObjectMeta.GenerateName = controllerName + "-pod-"
		if podTemplate.ObjectMeta.Labels == nil {
			podTemplate.ObjectMeta.Labels = map[string]string{}
		}
		podTemplate.ObjectMeta.Labels["name"] = controllerName

		if err := createController(client, controllerName, namespace, podCount, podTemplate); err != nil {
			return err
		}
		return CreatePod(client, namespace, podCount, podTemplate)
	}
